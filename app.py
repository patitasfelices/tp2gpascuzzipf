...
mascotas_schema = MascotaSchema(many=True)  # Objeto para serializar/deserializar múltiples mascotas
...
def get_Mascotas():
    ...
    result = mascotas_schema.dump(all_mascotas)  # Serializa los registros en formato JSON
...
def get_mascota(id):
    ...
    Retorna un JSON con la información del mascota correspondiente al ID proporcionado.
    """
    mascota = Mascota.query.get(id)  # Obtiene el mascota correspondiente al ID recibido
...
def delete_mascota(id):
    ...
    """
    mascota = Mascota.query.get(id)  # Obtiene el mascota correspondiente al ID recibido
    db.session.delete(mascota)  # Elimina el mascota de la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return mascota_schema.jsonify(mascota)  # Retorna el JSON del mascota eliminado
...
def create_mascota():
    ...
    id_especie = request.json["id_especie"]  # Obtiene el id_especie del mascota del JSON proporcionado
    peso = request.json["peso"]  # Obtiene el peso del mascota del JSON proporcionado
    imagen = request.json["imagen"]  # Obtiene la imagen del mascota del JSON proporcionado
    new_mascota = Mascota(nombre, id_especie, peso, imagen)  # Crea un nuevo objeto mascota con los datos proporcionados
    db.session.add(new_mascota)  # Agrega el nuevo mascota a la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return mascota_schema.jsonify(new_mascota)  # Retorna el JSON del nuevo mascota creado

@app.route("/mascotas/<id>", methods=["PUT"])  # Endpoint para actualizar un mascota
def update_mascota(id):
    ...
    Retorna un JSON con el mascota actualizado.
    """
    mascota = Mascota.query.get(id)  # Obtiene el mascota existente con el ID especificado
    ...
    db.session.commit()  # Guarda los cambios en la base de datos
    return mascota_schema.jsonify(mascota)  # Retorna el JSON del mascota actualizado
